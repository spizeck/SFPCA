...
// attention" read for staff. It composes three domains into ONE sorted
// action list:
//
...
import { followUpState } from "../medical";
import {
  addDaysToIsoDate,
  isIsoDateString,
  todayIsoDate,
...
function itemRank(item: VetQueueItem, asOf: string): number {
  ...
  const date = item.kind === "follow-up" ? item.dueOn : item.effectiveDate;
...
export async function listVetQueue(
  ...
  const [followUpRows, alertRows, dueVaccinations] = await Promise.all([
    db
      .select({
        id: followUps.id,
      ...
      .leftJoin(
        currentOwnership,
        eq(currentOwnership.animalId, animals.id),
      )
